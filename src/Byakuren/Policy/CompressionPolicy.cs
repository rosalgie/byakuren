...
public sealed class CompressionPolicy
{
    public IReadOnlyList<ResolvedPolicy> ResolveCandidates(CompressionRequest request)
    {
        ...
        if (requestedCodec != "auto" || requestedBackend != "auto") return [Resolve(request)];
        ...
        {
            "widest" => [("x264", "libx264")],
            "modern" when request.EnableExperimentalEncoders => [("av1", "svtav1"), ("vp9", "vpx"), ("x264", "libx264")],
            "modern" => [("av1", "svtav1"), ("x264", "libx264")],
            "unrestricted" when request.EnableExperimentalEncoders => [("x264", "libx264"), ("x265", "libx265"), ("av1", "svtav1"), ("vp9", "vpx")],
            "unrestricted" => [("x264", "libx264"), ("x265", "libx265"), ("av1", "svtav1")],
    ...
    public ResolvedPolicy Resolve(CompressionRequest request)
    {
        ...
        if (backend != "auto")
        {
            if (backend is "aom" or "rav1e" or "vpx" or "vvenc" && !request.EnableExperimentalEncoders)
                throw new ArgumentException($"Encoder backend '{backend}' is experimental; explicitly enable experimental encoders to use it.");
            if (backend == "vvenc")
        ...
        backend = backend == "auto" ? DefaultBackend(requestedCodec) : backend;
        container = container == "auto" ? DefaultContainer(requestedCodec) : container;
        string expectedContainer = DefaultContainer(requestedCodec);
        if (container != expectedContainer)
            throw new ArgumentException($"Codec '{requestedCodec}' with backend '{backend}' requires the '{expectedContainer}' delivery container.");
        ...
        return new ResolvedPolicy(profile, codecReason, request.Container.Equals("auto", StringComparison.OrdinalIgnoreCase) ? "codec-default" : "pinned", compatibility);
    }
    ...
    public static EncoderProfile CreateProfile(string codec, string backend, string container)
    {
        ...
        {
            ("libx264", "x264", "mp4") => Software("x264", "libx264", "libx264", "mp4", ".mp4", "aac", "aac", "x264"),
            ("libx265", "x265", "mp4") => Software("x265", "libx265", "libx265", "mp4", ".mp4", "aac", "aac", "x265"),
            ("svtav1", "av1", "webm") => Software("av1", "svtav1", "libsvtav1", "webm", ".webm", "opus", opus, "svtav1"),
            ("aom", "av1", "webm") => Software("av1", "aom", "libaom-av1", "webm", ".webm", "opus", opus, "aom"),
            ("rav1e", "av1", "webm") => Software("av1", "rav1e", "librav1e", "webm", ".webm", "opus", opus, "rav1e", passes: 1, adapter: "one-pass-vbr-lab"),
            ("vpx", "vp9", "webm") => Software("vp9", "vpx", "libvpx-vp9", "webm", ".webm", "opus", opus, "vpx"),
            ("vaapi", "x264", "mp4") => Hardware("x264", "h264_vaapi", "mp4", ".mp4", "aac", "aac"),
            ("vaapi", "x265", "mp4") => Hardware("x265", "hevc_vaapi", "mp4", ".mp4", "aac", "aac"),
            ("vaapi", "av1", "webm") => Hardware("av1", "av1_vaapi", "webm", ".webm", "opus", opus),
            _ => throw new ArgumentException($"Unsupported encoder/codec/container combination: {backend} + {codec} + {container}.")
        };
    }

    private static EncoderProfile Software(string codec, string backend, string encoder, string container, string extension, string audioCodec, string audioEncoder, string presetKind, int passes = 2, string adapter = "ffmpeg-two-pass-vbr") =>
        new()
        {
            VideoCodec = codec,
            Backend = backend,
    ...
    private static EncoderProfile Hardware(string codec, string encoder, string container, string extension, string audioCodec, string audioEncoder) =>
        new()
        {
            VideoCodec = codec,
            Backend = "vaapi",
    ...
    private static bool CodecAliasesMatch(string first, string second) => first == second || first == "h264" && second == "x264" || first == "hevc" && second == "x265";
