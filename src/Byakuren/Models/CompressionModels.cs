...
public enum TargetUnit { BinaryMiB, DecimalMB }
public enum SampleMode { Fixed, SceneAware, Auto }
public enum AudioPriority { Visual, Balanced, Speech }
public enum PreprocessMode { Off, Auto, Mild }
public enum CropMode { Off, Auto }
public enum VBVMode { Off, Streaming }
...
public sealed record CompressionPlan
{
    ...
    [JsonIgnore]
    public string Identity => $"{Profile.Backend}|{Width}x{Height}@{Fps:0.###}|{PixelFormat}|{VideoFilter}|{Preset}|{string.Join(':', Profile.PrivateArguments)}";
}
