...
public sealed class ContentAnalyzer(ProcessRunner runner)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (double startSeconds in SampleStarts(media.DurationSeconds))
        {
            ...
            {
                AddIfAvailable(entropyValues, MetadataAverage(baseProbe.CombinedOutput, "lavfi.entropy.normalized_entropy.normal.Y"));
                double? temporal = MetadataAverage(baseProbe.CombinedOutput, "lavfi.signalstats.YDIF");
                AddIfAvailable(temporalValues, temporal.HasValue ? temporal.Value / 255.0 : null);
                AddIfAvailable(noiseValues, MetadataAverage(baseProbe.CombinedOutput, "lavfi.signalstats.TOUT"));
            }
            else errors.Add(baseProbe.StandardError);

            ProcessResult shapeProbe = await runner.RunAsync(request.FFmpegPath,
            [
                .. commonArguments,
                "-filter_complex", "scale=320:-2:flags=area,edgedetect=low=0.08:high=0.20,format=gray,split[edge][flat];[edge]signalstats,metadata=print:file=-[measured];[flat]blackframe=amount=0:threshold=16,metadata=print:file=-[flatness]",
            ...
            if (shapeProbe.ExitCode == 0)
            {
                ...
                AddIfAvailable(edgeValues, edge.HasValue ? edge.Value / 255.0 : null);
                AddIfAvailable(flatValues, flat.HasValue ? flat.Value / 100.0 : null);
            }
            else errors.Add(shapeProbe.StandardError);
            ...
            else errors.Add(sceneProbe.StandardError);
        }
    ...
    public static string Classify(MediaInfo media, ContentFeatures features)
    {
        ...
        if (screenMotion || screenStatic) return "screen";
    ...
    {
        if (durationSeconds <= 2.0) return [0.0];
    ...
    private static double? MetadataAverage(string text, string key)
    {
        ...
        return matches.Select(match => double.Parse(match.Groups["value"].Value, CultureInfo.InvariantCulture)).Average();
    }

    private static double? SourceCompression(MediaInfo media)
    {
        if (media.VideoBitrateKbps <= 0 || media.Width <= 0 || media.Height <= 0 || media.Fps <= 0) return null;
        double bitsPerPixelPerFrame = media.VideoBitrateKbps * 1000.0 / (media.Width * (double)media.Height * media.Fps);
    ...
    private static string? LastUsefulError(IEnumerable<string> errors) =>
        errors.SelectMany(error => error.Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries)).LastOrDefault()?.Trim();
}
