...
public sealed class ComplexityAnalyzer(ProcessRunner runner)
{
    ...
    {
        int probeWidth = media.Width >= 1280 ? 480 : media.Width >= 854 ? 426 : Math.Min(media.Width, 360);
        ...
        int crf = request.Mode switch { CompressionMode.Fast => 32, CompressionMode.Balanced => 30, _ => 28 };
        string preset = request.Mode switch { CompressionMode.Fast => "superfast", CompressionMode.Balanced => "veryfast", _ => "fast" };

        IReadOnlyList<double> detailValues = await ProbeSeriesAsync(request, media, windows, tempDirectory, probeWidth, detailFps, crf, preset, "detail", cancellationToken).ConfigureAwait(false);
        IReadOnlyList<double> motionValues = motionFps > detailFps
            ? await ProbeSeriesAsync(request, media, windows, tempDirectory, probeWidth, motionFps, crf, preset, "motion", cancellationToken).ConfigureAwait(false)
            : detailValues;
    ...
    private static string DetailBucket(double kbps) => kbps switch { < 80 => "VeryLow", < 160 => "Low", < 320 => "Medium", < 650 => "High", _ => "VeryHigh" };
    private static string MotionBucket(double ratio, double normalized, double fps, string fallback) => fps <= 30.5 ? fallback : normalized switch { < 0.56 => "VeryLow", < 0.72 => "Low", < 0.92 => "Medium", < 1.12 => "High", _ => ratio > 1.8 ? "VeryHigh" : "High" };
    private static double Average(IReadOnlyList<double> values) => values.Count == 0 ? 0 : values.Average();
    private static double Spread(IReadOnlyList<double> values) => values.Count < 2 ? 0 : (values.Max() - values.Min()) / Math.Max(1, values.Average());
