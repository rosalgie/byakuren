...
public sealed class CropAnalyzer(ProcessRunner runner)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        bool bordersBlank = await BordersBlankAsync(request, media, offsets, frameWidth, frameHeight, x, y, width, height, cancellationToken).ConfigureAwait(false);
        if (!bordersBlank)
    ...
    private async Task<CropSample?> DetectSampleAsync(CompressionRequest request, MediaInfo media, double offset, CancellationToken cancellationToken)
    {
        ...
        MatchCollection matches = Regex.Matches(result.CombinedOutput, @"crop=(?<w>\d+):(?<h>\d+):(?<x>\d+):(?<y>\d+)");
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (right > 0) regions.Add((right, frameHeight, frameWidth - right, 0));
        if (y > 0) regions.Add((frameWidth, y, 0, 0));
        if (bottom > 0) regions.Add((frameWidth, bottom, 0, frameHeight - bottom));
        ...
        foreach (double offset in offsets)
        {
            foreach ((int regionWidth, int regionHeight, int regionX, int regionY) in regions)
            {
                ...
                string filter = $"crop={regionWidth}:{regionHeight}:{regionX}:{regionY},fps=4,signalstats,metadata=print:file=-";
                ProcessResult result = await runner.RunAsync(request.FFmpegPath,
                [
                ...
                if (yMaxValues.Count == 0 || yMinValues.Count == 0 || uMaxValues.Count == 0 || uMinValues.Count == 0 || vMaxValues.Count == 0 || vMinValues.Count == 0) return false;
                double scale = Math.Pow(2, Math.Max(0, media.BitDepth - 8));
                if (yMaxValues.Max() > 40 * scale || yMaxValues.Max() - yMinValues.Min() > 24 * scale) return false;
                if (uMinValues.Min() < 96 * scale || uMaxValues.Max() > 160 * scale || uMaxValues.Max() - uMinValues.Min() > 32 * scale) return false;
                if (vMinValues.Min() < 96 * scale || vMaxValues.Max() > 160 * scale || vMaxValues.Max() - vMinValues.Min() > 32 * scale) return false;
            }
        }
        return true;
    }

    private static IReadOnlyList<double> Values(string output, string key) => Regex.Matches(output, Regex.Escape(key) + @"=(?<value>\d+(?:\.\d+)?)")
        .Select(match => double.Parse(match.Groups["value"].Value, CultureInfo.InvariantCulture)).ToArray();
    private static IReadOnlyList<double> Offsets(double duration, int sampleSeconds, int maxSamples) => SampleWindowPlanner.FixedWindows(duration, sampleSeconds, maxSamples).Select(window => window.StartSeconds).ToArray();
