...
public sealed class FFmpegEncoder(ProcessRunner runner, FFmpegProbe probe)
{
    ...
    {
        if (!media.HasAudio || audioPlan.Mode == "mute" || audioPlan.Kbps <= 0)
            return new AudioArtifact(null, 0, AudioPlan.Mute);
        if (audioPlan.Mode == "copy")
        {
            ...
            long copyPayload = await probe.GetPacketPayloadBytesAsync(
                request.FFprobePath,
                copyPath,
                "a:0",
                cancellationToken).ConfigureAwait(false);
            ...
            if (copyPayload <= 0)
                copyPayload = new FileInfo(copyPath).Length;

            return new AudioArtifact(copyPath, copyPayload, audioPlan);
        }
        ...
        long payload = await probe.GetPacketPayloadBytesAsync(
            request.FFprobePath,
            path,
            "a:0",
            cancellationToken).ConfigureAwait(false);
        ...
        if (payload <= 0)
            payload = new FileInfo(path).Length;
        ...
        CancellationToken cancellationToken)
    {
        ...
        bool useTwoPass = plan.Profile.RequiredPasses >= 2 &&
            (plan.Mode != CompressionMode.Fast ||
             plan.Profile.Backend is "svtav1" or "aom" or "vpx");
        if (useTwoPass)
        {
        ...
        long videoPayload = await probe.GetPacketPayloadBytesAsync(
            request.FFprobePath,
            videoPath,
            "v:0",
            cancellationToken).ConfigureAwait(false);
        ...
        if (videoPayload <= 0)
            videoPayload = new FileInfo(videoPath).Length;
        ...
        foreach (AudioArtifact muxAudio in muxAudioCandidates)
        {
            string candidatePath = Path.Combine(
                tempDirectory,
                $"candidate-{attempt}-{muxIndex++}{plan.Profile.Extension}");
            ...
            mux.AddRange(["-c", "copy"]);
            if (plan.Profile.Container == "mp4")
                mux.AddRange(["-movflags", "+faststart"]);
            ...
            long candidateOverhead = Math.Max(
                0,
                candidateSize - videoPayload - muxAudio.PayloadBytes);
        ...
        (string Path, long Size, long Overhead, AudioArtifact Audio)? eligible = muxedCandidates
            .Where(candidate =>
                candidate.Size <= plan.HardCapBytes &&
                candidate.Size / (double)plan.HardCapBytes >= fillGate)
            .OrderByDescending(candidate => candidate.Size)
            .Cast<(string Path, long Size, long Overhead, AudioArtifact Audio)?>()
            .FirstOrDefault();

        (string Path, long Size, long Overhead, AudioArtifact Audio) selected = muxedCandidates[0];
        if (attempt >= strategy.MaxFullEncodes && eligible.HasValue)
        ...
        CompressionPlan selectedPlan = plan with
        {
            AudioPlan = selected.Audio.Plan,
            AudioKbps = selected.Audio.Plan.Kbps
        };
        ...
        CancellationToken cancellationToken)
    {
        ...
        [
            "-y", "-ss", Number(window.StartSeconds),
            "-t", Number(window.DurationSeconds),
            "-i", media.Path
        ];
    ...
    private static IReadOnlyList<string> BuildVideoArguments(CompressionPlan plan, string hardwareDevice)
    {
        ...
        if (plan.Profile.IsHardware)
            arguments.AddRange(["-vaapi_device", hardwareDevice]);
        if (!string.IsNullOrWhiteSpace(plan.VideoFilter))
            arguments.AddRange(["-vf", plan.VideoFilter]);
        arguments.AddRange(["-c:v", plan.Profile.Encoder]);
        arguments.AddRange(CapabilityProbe.PresetArguments(plan.Profile, plan.Preset));
        if (!plan.Profile.IsHardware)
            arguments.AddRange(["-pix_fmt", plan.PixelFormat]);
        arguments.AddRange(["-b:v", $"{plan.VideoKbps}k"]);
        if (plan.MaxrateKbps.HasValue)
            arguments.AddRange(["-maxrate", $"{plan.MaxrateKbps.Value}k"]);
        if (plan.BufsizeKbits.HasValue)
            arguments.AddRange(["-bufsize", $"{plan.BufsizeKbits.Value}k"]);
