...
public sealed class ResultContract
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        long reservedMuxBytes = plan?.MuxReserveBytes ?? 0;
        bool exceededMuxReserve = action == "encode" &&
            attempt is not null &&
            attempt.MuxOverheadBytes > Math.Max(4096, reservedMuxBytes);
        if (exceededMuxReserve)
        {
            warnings.Add(
                $"mux overhead used {attempt!.MuxOverheadBytes} bytes, above the reserved " +
        ...
        string hostText = $"{RuntimeInformation.OSDescription}|" +
            ...
            $"{capability?.Driver}|{capability?.FFmpegBuild}";

        long workingTargetBytes = plan?.WorkingTargetBytes ??
            (long)Math.Floor(request.TargetBytes * request.WorkingTargetRatio);
        string gpu = "";
        if (capability?.Backend == "vaapi")
            gpu = capability.Device;

        string primaryMetric = "";
        if (metrics.VMAFNeg.HasValue)
            primaryMetric = "vmaf-neg";
        else if (metrics.XPSNR.HasValue)
            primaryMetric = "xpsnr";

        IReadOnlyList<string> encoderArguments = [];
        if (plan is not null)
            encoderArguments = BuildEncoderArguments(plan);

        double? fillGate = null;
        if (action == "encode")
            fillGate = strategy.FillGate;

        object capabilityProbe = capability ?? new
        {
            Success = (bool?)null,
        ...
        return new
        {
            ...
            Request = new
            {
                ...
                WorkingTargetBytes = workingTargetBytes,
